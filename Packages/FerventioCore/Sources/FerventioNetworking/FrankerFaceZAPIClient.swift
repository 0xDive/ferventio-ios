...
public struct FrankerFaceZAPIClient: Sendable {
    ...
    private struct Room: Decodable, Sendable {
        let set: Int?
    }

    private struct EmoteSet: Decodable, Sendable {
        let emoticons: [Emote]
    }
    ...
    public func getGlobalEmotes() async throws -> [ThirdPartyEmoteDefinition] {
        try await perform(Self.makeGlobalRequest(), selection: .global)
    }

    public func getChannelEmotes(twitchUserID: String) async throws -> [ThirdPartyEmoteDefinition] {
        try await perform(
            Self.makeChannelRequest(twitchUserID: twitchUserID),
            selection: .channel
        )
    }

    static func makeGlobalRequest() -> URLRequest {
        URLRequest(url: URL(string: "https://api.frankerfacez.com/v1/set/global")!)
    }
    ...
    private func perform(
        ...
        selection: Selection
    ) async throws -> [ThirdPartyEmoteDefinition] {
        var request = request
        ...
        guard (200..<300).contains(httpResponse.statusCode) else {
            ...
            throw Error.httpStatus(
                ...
                body?.isEmpty == false ? body! : "unknown FrankerFaceZ error"
            )
        }

        switch selection {
        case .global:
            return try Self.parseGlobalResponse(data)
        case .channel:
            return try Self.parseChannelResponse(data)
        }
    }
    ...
    private static func mapEmote(_ emote: Emote) -> ThirdPartyEmoteDefinition? {
        ...
        // FFZ modifier emotes have suffix semantics and effect bitsets. Until
        // the dedicated FFZ modifier renderer is wired, keep them out of the
        // text replacement catalog instead of rendering them incorrectly.
        guard emote.modifier != true else {
            return nil
        }

        let animatedURL = preferredURL(emote.animated)
        let staticURL = preferredURL(emote.urls)
        guard let imageURL = animatedURL ?? staticURL else {
            return nil
        }
    ...
    private static func preferredURL(_ urls: [String: String]?) -> String? {
        ...
        for scale in ["2", "4", "1"] {
            if let value = urls[scale], let resolved = absoluteURL(value) {
                return resolved
            }
        }
        return nil
